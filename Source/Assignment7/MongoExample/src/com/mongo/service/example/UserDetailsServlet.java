...
public class UserDetailsServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		MongoClientURI uri = new MongoClientURI("mongodb://teju:1234@ds019628.mlab.com:19628/teju");
		MongoClient client = new MongoClient(uri);

		DB db = client.getDB(uri.getDatabase());
		DBCollection users = db.getCollection("sample");
		BasicDBObject query = new BasicDBObject().append("name","teja");
		

		BasicDBObject newDocument = new BasicDBObject();
		newDocument.put("name","Tejaswi");
		

		users.update(query,newDocument);
		
		
		query.put("name","Tejaswi");// request.getParameter("name"));
		//query.put("password","5678"); //request.getParameter("password"));
